...
### 3.4 Notion 페이지 오타 검증 (반드시 시행)

Notion 페이지 업로드 직후 반드시 검증 단계를 거친다. IME 변환 사고 / 마크다운 escape 충돌 / 시각적으로 유사한 한글 자모 (예: `볗` U+BCD7 vs U+BCF3) 로 인한 오타가 흔히 발생한다.

1. **fetch 로 페이지 다시 읽기** — `mcp__notion__notion-fetch` 로 업로드된 raw 마크다운을 읽음. 작성 시 본 것과 다르게 저장될 수 있음 (특히 `*` / `**` escape 처리).

2. **검증 항목** — 다음 다섯 가지 패턴 위주로 훑는다:
   - **한국어 자모 오타** — 비단어 (예: "동달자" → "디자이너"), 시각적 유사 문자 (`컬` U+CEEC vs `컴` U+CEF4, `볗` 계열 등)
   - **비문법 / IME 변환 사고** — 어색한 활용형 ("바꿔다" → "바꾼다"), 받침 불일치, 띄어쓰기로 잘려 합쳐진 글자
   - **마크다운 깨짐 — BFS 식 패턴** — `**A\\*** 가 *어떻게\\*` 같이 italic / bold 닫음이 escape 와 충돌. 특히 `A*`, `O(b^d)` 같이 별표/캐럿 포함 텍스트 주변에서 자주 발생
   - **숫자 / 카운트 일관성** — 표의 행 수와 본문 카운트 (예: "행동 카탈로그 12개 — Move 5종 + 상호작용 7종" 인데 표에 행이 6개) 일치 여부
   - **Mermaid 다이어그램** — 노드 라벨, 화살표 텍스트에 오타 흔함 (`인접 볗어남` → `인접 벗어남`)

3. **수정 방법** — `mcp__notion__notion-update-page` 의 `update_content` 명령 + `content_updates` 배열로 일괄 search-and-replace.
   - 같은 패턴 여러 곳은 `replace_all_matches: true`
   - 자모가 미묘하게 다른 문자는 codepoint 직접 확인 (`U+ABCD` 형식). 시각으로는 동일해 보여도 다른 문자.
   - `old_str` 과 `new_str` 이 같은 문자로 끝나면 (예: 둘 다 `와`) replace 가 무효 — 확인 필수.

4. **API 로 안 잡히는 패턴** — `*X\*` BFS 식 마크다운 깨짐은 API 가 매치 못하는 경우가 잦다. Notion UI 에서 수동 수정 권장 (사용자에게 위치 안내).

5. **모호한 단어는 사용자 확인** — 의도 불분명한 비단어 (예: "탐돈도", "차도의") 는 추측하지 말고 후보 2~3 개 제시 후 사용자 선택 받기. 의미 보존이 우선.

6. **최종 검증** — 수정 후 `notion-fetch` 로 다시 읽어 모든 변경이 적용됐는지 확인. 실패한 항목은 별도로 보고.

> **참고** : 사용자는 raw 마크다운이 아닌 Notion UI 렌더링을 본다. raw 에서 어색해 보여도 UI 에서 정상 렌더링되면 무시 가능. 의심스러우면 사용자에게 "UI 에서 어떻게 보이는지" 확인 요청.

### 3.5 프로젝트 루트 README.md (반드시 갱신)
